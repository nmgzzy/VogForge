    ...
    <div className="rounded-lg border border-line bg-panel p-4">
      ...
      <div className="mt-3 flex items-center justify-between gap-2 text-xs text-muted">
        ...
        <span className="flex min-w-0 items-center gap-1" title={plan.video.encoder}>
          ...
          <span className="truncate">{copy ? "流复制" : hw ? VENDOR_LABEL[encoderVendor(plan.video.encoder)] : "CPU 软编"}</span>
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-lg border border-line bg-panel">
      <header className="flex h-10 items-center gap-2 border-b border-line px-4">
        ...
        <h2 className="text-[13px] font-semibold">推荐说明</h2>
        {hidden > 0 && (
...
function DecisionRow({ d, expanded, onToggle }: { d: Decision; expanded: boolean; onToggle?: () => void }) {
  ...
  return (
    <li className={cn("relative px-4 py-1.5", warn && "bg-warn/[0.05]")}>
      ...
      >
        <span className="w-12 shrink-0 text-[11px] text-subtle">{d.field}</span>
      ...
      {expanded && <p className="mt-0.5 mb-0.5 pl-14 text-[11.5px] leading-relaxed text-muted">{d.reason}</p>}
    </li>
  );
}
