...
对全项目做了一次整体审查（自查加 Codex），确认"不升档"规则，并修掉发现的问题：

- [x] 不升档：固定帧率的目标高于源时拉回源帧率（界面置灰更高的档位，源帧率不在标准档里时单独列出）；目标码率不高于源视频码率、限峰值的峰值不高于它的 1.5 倍（专家面板显示源码率，输入更高的值会拉回）；重新编码的音轨码率不高于有损源，单声道不升立体声，3–5 声道不升 5.1。分辨率原本就不放大
- [x] 降帧率时推荐理由说明丢帧（原来写"输出保持一致"）
- [x] 降混：`pan` 引用不存在的声道会静默丢掉（实测），只对确切已知的 5.x / 7.x 布局用 pan 矩阵，其余布局用默认矩阵
- [x] 附加参数里出现位置参数（会变成又一个输出并被 `-y` 覆盖）或 `-i` / `-y` / `-n` / `-progress` 时整段不用，并给出警告
- [x] 封面图单独计数：推荐理由提醒不保留，校验报告标出，校验不通过的源文件不会被移进回收站
- [x] 校验报告核对重新编码音轨的编码与声道数
- [x] 命名模板的 `{date}`：加入队列时带上界面的本地日期，预览与实际文件名一致（原来后端取 UTC 日期，东八区 0–8 点会差一天）
- [x] 命令栏文案改为"编码成功后改名，再校验"，与实际顺序一致
- [x] 专家面板的码率输入框：提交的值被拉回后，输入框显示实际值（原来数值没变时停在用户输入的值上）
- [ ] 保留封面图：把视频参数改成按流限定（`-c:v:0`、`-filter:v:0`、`-tag:v:0`……）后映射封面流并设 `attached_pic`，MKV 与 MP4 各实测一次
- [ ] 音频逐轨手动调整（需求 F-3.5）：界面只有三种整体音频模式，`normalize_plan` 每次都重建音轨；需要区分自动生成与用户锁定的逐轨设置
- [ ] 校验阶段的 ffprobe 也走可暂停、可取消、随父进程结束的进程（现在只有数帧走），慢速网络盘上取消要等它超时
- [ ] 改名完成、校验结束前崩溃：恢复时先核对已有的最终文件，通过就直接完成，而不是重新转码（现在会按冲突策略多出一个 `(1)` 文件）
- [ ] 队列状态保存失败（磁盘满、权限）现在被忽略：应在界面提示，并在保存成功前不开始新任务

## Win10 反馈：添加文件夹后没有加入视频（2026-09-14）

- [x] 扩展名表补上 rmvb / rm / f4v / asf / 3g2 / ogv / m2t / m2v / divx / dv / mod / tod / mk3d / insv / 360（Rust 与前端文件对话框两份同步）
- [x] 导入报告列出被跳过的扩展名；一个都没加进来时写明扫的是哪个文件夹；与文件无关的失败（没有 ffprobe）标题写"无法导入"
- [x] 文件夹对话框标题提示手机等设备里的视频要先拷到电脑上
- [ ] 确认用户场景后再定：自己实现 Windows 文件夹对话框并设 `FOS_FORCEFILESYSTEM`，让对话框直接拒绝手机、库这类没有路径的位置（rfd 选中它们时静默返回空）

## RTX 4070 反馈：AV1 硬件编码体积与源相当（2026-09-14）

- [x] `av1_nvenc` 的 `-cq` 刻度是 0–63、`av1_amf` 的 QP 是 0–255，引擎原来按同家族 HEVC 的 0–51 给值，高画质档接近无损。改为按编码器单独给刻度与档位（av1_nvenc 24 / 30 / 35 / 40，av1_amf 96 / 120 / 140 / 160），界面的数值范围随之变化
- [x] 单元测试：每个编码器每一档都在自己的刻度里、单调、高画质档不落在最细的一成
- [ ] 在 NVIDIA 与 AMD 真卡上校准 AV1 硬件编码的档位（开发机只有 Intel 核显，这两组数值按刻度换算，没有实测），并核对体积预估
- [ ] `av1_qsv` 与 `hevc_qsv` 取相同 `global_quality` 时 AV1 码率反而更高（加噪测试图 14.9 vs 9.6 Mbps），用真实素材确认是否需要单独调档
