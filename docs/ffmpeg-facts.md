...
[实测] `pan` 引用输入布局里没有的声道**不报错，而是静默忽略**（9.0.1，`-v verbose` 可以看到这些声道的系数被丢掉）。用 5.1 的矩阵（BL/BR）处理 6.1（FL FR FC LFE BC SL SR），只在 SL 放 440 Hz 正弦，输出平均音量 −91 dB，等于静音；同一个源用 `-ac 2` 是 −11.5 dB。7.1 的矩阵处理 7.1(wide)（中置两侧是 FLC / FRC，没有 SL / SR）同样丢掉 FLC / FRC。所以矩阵只能用在声道名确切已知的布局上：5.0 / 5.1 / 7.0 / 7.1 与 5.0(side) / 5.1(side)。其余布局写 `aformat=channel_layouts=stereo`，让自动插入的 aresample 按默认矩阵混入每个声道：6.1 只放 SL、7.1(wide) 只放 FLC、4.0 只放 BC 三个样本分别是 −9.1 / −6.3 / −9.1 dB，输出 2 声道。写成滤镜而不是 `-ac 2`，响度测量与编码两遍才会看到同样的立体声。
