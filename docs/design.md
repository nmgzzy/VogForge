...
色深与字幕通常由场景决定，放在 L3。质量选择在 L2 用语义档位（视觉无损 / 高 / 标准 / 小体积），在 L3 才露出具体数值。质量档位到数值的映射按编码器分别定义，并在界面标注"跨编码器不等价"——x265 CRF 18、NVENC CQ 26、QSV global_quality 24 之间没有统一刻度。同一家族里 AV1 硬件编码器的刻度也不同：`av1_nvenc` 的 CQ 是 0–63、`av1_amf` 的 QP 是 0–255，要单独映射（技术事实文档 8.2）。
